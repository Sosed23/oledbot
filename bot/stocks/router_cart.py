...
@cart_router.callback_query(F.data.startswith("cart_web_re-gluing_"))
async def handle_web_re_gluing(callback: CallbackQuery, state: FSMContext):
    logger.debug(f"Вызван handle_web_re_gluing с callback_data: {callback.data}")
    try:
        # Извлекаем model_id и model_name из callback_data
        data = callback.data.split("_")
        ...
        logger.error(f"Ошибка в handle_web_re_gluing: {e}")
        result = await callback.message.answer("Произошла ошибка при обработке данных.")
        await callback.answer()
        return result
